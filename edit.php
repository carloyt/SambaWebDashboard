...
<html lang="fr">
	<head>
		<title>Editer les partages de Samba<?php if( isset($_GET['action']) ) { if ( $_GET['action'] == 'edit' ) echo ' - ' . $_GET['file']; } ?></title>
	...
	if ( $_GET['action'] == 'see' ) {
		...
		while ($i <= $nblignes) {
			...
			echo '
				...
				. $fname . 
				' target="_blank" >Modifier ce contenu</a> - 
				<a class="del_share" href=?action=cedit&file='
				. $fname . 
				'&ligne=' . $lignesmb . ' target="_blank" >Supprimer ce contenu</a></p>
			';
